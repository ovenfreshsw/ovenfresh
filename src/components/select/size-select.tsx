        ...
        setOrderItems((prevItems) => {
            ...
            const updatedItems = prevItems.map((item) => {
                if (item.itemId._id === itemId && item.size === defaultSize) {
                    ...
                    return {
                        ...
                        size: value,
                        priceAtOrder:
                            menu[`${value}Price` as "smallPrice"] || 0,
                    };
                }
                return item;
            });
            ...
            const finalItems = updatedItems.reduce((acc: ItemProps[], item) => {
                const existingItem = acc.find(
                    (i) =>
                        i.itemId._id === item.itemId._id && i.size === item.size
                );
        ...
        <Select defaultValue={defaultSize} onValueChange={handleValue}>
            ...
            <SelectContent>
                {filteredItems.map((size, i) => (
                    <SelectItem value={size} className="capitalize" key={i}>
                        {size}
                    </SelectItem>
                ))}
            </SelectContent>
...
function filterItems(item: CateringMenuDocument) {
    const filteredItems = [];
    if (item.smallPrice) filteredItems.push("small");
    if (item.mediumPrice) filteredItems.push("medium");
    if (item.largePrice) filteredItems.push("large");
